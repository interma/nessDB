...
struct sst *sst_new(const char *file, struct stats *stats)
{
	...
	sst->w_lock = xmalloc(sizeof(pthread_mutex_t));
	pthread_mutex_init(sst->w_lock, NULL);
...
{
	pthread_mutex_unlock(sst->w_lock);
	pthread_mutex_destroy(sst->w_lock);
	xfree(sst->w_lock);
